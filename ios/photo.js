...
const save=()=>{
 if(!processed)return;
 setStatus('Підготовка PNG…');
 result.toBlob(async blob=>{
   if(!blob){setStatus('Не вдалося створити PNG.');return;}
   const name='Meti-Tuman-MAX-'+Date.now()+'.png';
   try{
     const file=new File([blob],name,{type:'image/png'});
     if(navigator.canShare?.({files:[file]})&&navigator.share){
       await navigator.share({files:[file],title:'Меті Туман — Фото MAX'});
       setStatus('Відкрито збереження або поширення.');
     }else{
       const url=URL.createObjectURL(blob),a=document.createElement('a');
       a.download=name;a.href=url;document.body.appendChild(a);a.click();a.remove();
       setTimeout(()=>URL.revokeObjectURL(url),20000);
       setStatus('PNG підготовлено. За потреби натисни Поділитися → Зберегти зображення.');
     }
   }catch(err){
     if(err.name!=='AbortError')setStatus('Не вдалося зберегти: '+err.message);
   }
 },'image/png');
};
