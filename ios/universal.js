...
const state={
 ...
 installedEvent:null,usingCpu:false,hybridEnabled:true,hybridAvailable:true
};
...
let hybridWorker=null,hybridBusy=false,hybridReady=null,hybridCurrent=null,hybridMapTex=null,hybridLutTex=null,hybridShader=false;
let hybridAnalysis=document.createElement('canvas');hybridAnalysis.width=192;hybridAnalysis.height=108;
const hybridCtx=hybridAnalysis.getContext('2d',{willReadFrequently:true});
let hybridEpoch=0,lastHybridAnalysis=0,hybridProcessingMs=0;
...
function syncUI(){
 ...
 $('hybridEnabled').checked=state.hybridEnabled;
 $('hybridEnabled').disabled=!state.hybridAvailable;
 ...
 $('autoBadge').textContent=(state.hybridEnabled&&state.hybridAvailable?'MAX ':'FAST ')+(state.manual?'РУЧНИЙ ':'AUTO ')+
...
$('hybridEnabled').onchange=e=>{state.hybridEnabled=e.target.checked;syncUI();renderFrame(true);};
...
function initializeHybridWorker(){
 if(hybridWorker||!hybridShader||!state.hybridAvailable)return;
 try{
  hybridWorker=new Worker('./video-worker.js');
  hybridWorker.onmessage=event=>{
   hybridBusy=false;
   const data=event.data;
   if(data.epoch!==hybridEpoch)return;
   if(data.error){
     state.hybridAvailable=false;state.hybridEnabled=false;
     showNotice('VIDEO MAX недоступний: '+data.error+'. Увімкнено швидкий фільтр.',6500);
     syncUI();return;
   }
   hybridReady={map:new Uint8Array(data.map),lut:new Uint8Array(data.lut),
      air:data.air,haze:data.haze,elapsed:data.elapsed};
   hybridProcessingMs=data.elapsed;
  };
  hybridWorker.onerror=e=>{
   hybridBusy=false;state.hybridAvailable=false;state.hybridEnabled=false;
   showNotice('Фоновий алгоритм VIDEO MAX недоступний. Працює FAST.',5500);
   syncUI();
  };
 }catch(error){
  state.hybridAvailable=false;state.hybridEnabled=false;syncUI();
 }
}
function installHybridTextures(){
 hybridMapTex=gl.createTexture();
 gl.activeTexture(gl.TEXTURE1);gl.bindTexture(gl.TEXTURE_2D,hybridMapTex);
 gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_WRAP_S,gl.CLAMP_TO_EDGE);
 gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_WRAP_T,gl.CLAMP_TO_EDGE);
 gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MIN_FILTER,gl.LINEAR);
 gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MAG_FILTER,gl.LINEAR);
 gl.texImage2D(gl.TEXTURE_2D,0,gl.RGBA,192,108,0,gl.RGBA,gl.UNSIGNED_BYTE,null);
 hybridLutTex=gl.createTexture();
 gl.activeTexture(gl.TEXTURE2);gl.bindTexture(gl.TEXTURE_2D,hybridLutTex);
 gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_WRAP_S,gl.CLAMP_TO_EDGE);
 gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_WRAP_T,gl.CLAMP_TO_EDGE);
 gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MIN_FILTER,gl.NEAREST);
 gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MAG_FILTER,gl.NEAREST);
 gl.texImage2D(gl.TEXTURE_2D,0,gl.RGBA,256,48,0,gl.RGBA,gl.UNSIGNED_BYTE,null);
 gl.uniform1i(gl.getUniformLocation(program,'transmission'),1);
 gl.uniform1i(gl.getUniformLocation(program,'clahe'),2);
 gl.activeTexture(gl.TEXTURE0);
}
function uploadHybrid(){
 if(!gl||!hybridShader)return;
 if(hybridReady){
  hybridCurrent=hybridReady;hybridReady=null;
  gl.activeTexture(gl.TEXTURE1);gl.bindTexture(gl.TEXTURE_2D,hybridMapTex);
  gl.texSubImage2D(gl.TEXTURE_2D,0,0,0,192,108,gl.RGBA,gl.UNSIGNED_BYTE,hybridCurrent.map);
  gl.activeTexture(gl.TEXTURE2);gl.bindTexture(gl.TEXTURE_2D,hybridLutTex);
  gl.texSubImage2D(gl.TEXTURE_2D,0,0,0,256,48,gl.RGBA,gl.UNSIGNED_BYTE,hybridCurrent.lut);
  gl.activeTexture(gl.TEXTURE0);
 }
 gl.uniform1f(uniforms.hybrid,(state.hybridEnabled&&hybridCurrent&&state.hybridAvailable)?1:0);
 if(hybridCurrent)gl.uniform3fv(uniforms.air,new Float32Array(hybridCurrent.air));
}
function scheduleHybrid(now){
 if(!hybridWorker||!hybridCtx||!state.enabled||!state.hybridEnabled||
    !state.hybridAvailable||hybridBusy||video.paused||video.readyState<2||
    now-lastHybridAnalysis<600)return;
 lastHybridAnalysis=now;
 try{
  hybridCtx.drawImage(video,0,0,192,108);
  const frame=hybridCtx.getImageData(0,0,192,108);
  hybridBusy=true;
  hybridWorker.postMessage({frame:frame.data.buffer,epoch:hybridEpoch},[frame.data.buffer]);
 }catch(error){
  hybridBusy=false;state.hybridAvailable=false;state.hybridEnabled=false;
  showNotice('MAX вимкнено: браузер не дозволив аналіз відео. FAST залишається.',5500);
  syncUI();
 }
}
function initGpu(){
 ...
 try{
   ...
   const vs=shader(gl.VERTEX_SHADER,`attribute vec2 aPosition;
       varying vec2 uv;
       void main(){uv=(aPosition+1.0)*.5;gl_Position=vec4(aPosition,0.0,1.0);}`);
   const regularBody=`precision mediump float;
       varying vec2 uv;
       ...
       void main(){
         ...
         vec3 local=(texture2D(tex,uv+vec2(pixel.x*2.0,0.0)).rgb+
          ...
          texture2D(tex,uv-vec2(0.0,pixel.y*2.0)).rgb)*.25;
   ...
   const hybridBody=`precision mediump float;
       varying vec2 uv;
       uniform sampler2D tex;
       uniform sampler2D transmission;
       uniform sampler2D clahe;
       uniform vec3 air;
       uniform vec2 pixel;
       uniform float intensity;
       ...
       void main(){
         ...
         if(hybrid>.5){
           vec4 m=texture2D(transmission,uv);
           float t=max(.26,m.r);
           float blend=clamp(intensity*1.02*(1.0-.91*m.g),0.0,.96);
           vec3 restored=clamp((c-air)/t+air,0.0,1.0);
           vec3 outColor=mix(c,restored,blend);
           float lum=dot(outColor,vec3(.299,.587,.114));
           vec2 pos=clamp(uv*vec2(8.0,6.0)-.5,vec2(0.0),vec2(7.0,5.0));
           vec2 low=floor(pos),high=min(low+vec2(1.0),vec2(7.0,5.0));
           vec2 f=fract(pos);
           float bin=floor(clamp(lum,0.0,1.0)*255.0+.5);
           float a=texture2D(clahe,vec2((bin+.5)/256.0,(low.y*8.0+low.x+.5)/48.0)).r;
           float b=texture2D(clahe,vec2((bin+.5)/256.0,(low.y*8.0+high.x+.5)/48.0)).r;
           float d=texture2D(clahe,vec2((bin+.5)/256.0,(high.y*8.0+low.x+.5)/48.0)).r;
           float e=texture2D(clahe,vec2((bin+.5)/256.0,(high.y*8.0+high.x+.5)/48.0)).r;
           float mapped=mix(mix(a,b,f.x),mix(d,e,f.x),f.y);
           float delta=clamp(mapped-lum,-.18,.18)*.39*blend*(1.0-m.g);
           outColor=clamp(outColor+vec3(delta),0.0,1.0);
           outColor=clamp(outColor+clamp(c-local,vec3(-.09),vec3(.09))*(.35*blend*m.a),0.0,1.0);
           gl_FragColor=vec4(outColor,1.0);return;
         }
         float edge=length(c-local);
         float mask=1.0-.85*smoothstep(.69,.91,l)*(1.0-smoothstep(.01,.07,edge));
         float t=max(.58,1.0-intensity*(.25+.18*l));
         vec3 rec=clamp((c-vec3(.81))/t+vec3(.81),0.0,1.0);
         vec3 corrected=mix(c,rec,.79*mask);
         corrected+=clamp(c-local,vec3(-.10),vec3(.10))*(.48*intensity*mask);
         gl_FragColor=vec4(clamp(corrected,0.0,1.0),1.0);
   ...
   let fs;
   try{
     fs=shader(gl.FRAGMENT_SHADER,hybridBody);
     hybridShader=true;
   }catch(error){
     console.warn('VIDEO MAX shader unavailable, using FAST:',error);
     state.hybridAvailable=false;state.hybridEnabled=false;hybridShader=false;
     fs=shader(gl.FRAGMENT_SHADER,regularBody);
   }
   ...
   if(hybridShader){
     uniforms.hybrid=gl.getUniformLocation(program,'hybrid');
     uniforms.air=gl.getUniformLocation(program,'air');
     installHybridTextures();
     initializeHybridWorker();
   }
   syncUI();
 }catch(error){
   console.warn('WebGL unavailable:',error);
   gl=null;state.usingCpu=true;state.hybridAvailable=false;state.hybridEnabled=false;
   ...
   updateMediaGeometry();syncUI();
   showNotice('Увімкнено сумісний режим CPU. VIDEO MAX недоступний на цьому пристрої.',6500);
 }
}
...
function renderFrame(force=false){
 ...
 try{
   ...
   if(gl){
     ...
     if(hybridShader)uploadHybrid();
     gl.drawArrays(gl.TRIANGLE_STRIP,0,4);
     if(hybridShader)scheduleHybrid(now);
   }else if(canvas2d){
...
async function cleanup(){
 ...
 hybridEpoch++;hybridReady=null;hybridCurrent=null;lastHybridAnalysis=0;
...
$('quickPhoto').onclick=$('startPhoto').onclick=$('photoMenu').onclick=()=>{location.href='./photo.html';};
   ...
   out.toBlob(async blob=>{
     ...
     try{
       ...
       if(navigator.canShare?.({files:[file]})&&navigator.share){
         await navigator.share({files:[file],title:'Меті Туман'});
       }else{
...
async function updateOffline(){
 ...
 try{
   ...
   label.textContent=keys.some(k=>k.startsWith('meti-ios-offline-'))
